...
class TestMetadata:
    ...
    def test_default_model(self, provider):
        assert provider.default_model() == "gpt-image-2.5-flare-medium"

    def test_list_models_spans_both_generations(self, provider):
        ids = [m["id"] for m in provider.list_models()]
        assert len(ids) == 8
        assert "gpt-image-2.5-flare-medium" in ids
        assert "gpt-image-2.5-sunburst-high" in ids
        assert "gpt-image-2-high" in ids  # legacy generation retained
...
class TestGenerate:
    ...
    def test_generate_uses_codex_stream_path(self, provider, monkeypatch, tmp_path):
        ...
        assert result["model"] == "gpt-image-2.5-flare-medium"
        assert result["provider"] == "openai-codex"
        assert result["quality"] == "medium"
        assert result.get("api_model") == "gpt-image-2.5-flare"
        assert result.get("background") == "opaque"
    ...
    def test_codex_stream_request_shape(self, provider, monkeypatch):
        ...
        def _collect(token, *, prompt, size, quality, input_images=None, api_model=None, background=None):
            captured.update(codex_plugin._build_responses_payload(
                prompt=prompt,
                size=size,
                quality=quality,
                input_images=input_images,
                api_model=api_model or codex_plugin._API_MODEL_FLARE,
                background=background or "opaque",
            ))
        ...
        assert tool["model"] == "gpt-image-2.5-flare"
...
# ── GPT-Image-2.5 catalog expansion (2026-09-10) ────────────────────────────


class TestGPTImage25Catalog:
    def test_every_tier_pins_api_model(self):
        for tier_id, meta in codex_plugin._MODELS.items():
    ...
    def test_legacy_tier_id_still_resolves(self, monkeypatch):
        monkeypatch.setenv("OPENAI_IMAGE_MODEL", "gpt-image-2-high")
        tier, meta = codex_plugin._resolve_model()
        assert tier == "gpt-image-2-high"
        assert meta["api_model"] == "gpt-image-2"

    def test_sunburst_tier_resolves(self, monkeypatch):
        monkeypatch.setenv("OPENAI_IMAGE_MODEL", "gpt-image-2.5-sunburst-high")
        tier, meta = codex_plugin._resolve_model()
        assert meta["api_model"] == "gpt-image-2.5-sunburst"

    def test_tool_config_pins_api_model(self):
        payload = codex_plugin._build_responses_payload(
            prompt="a cat", size="1024x1024", quality="high",
            api_model="gpt-image-2.5-sunburst", background="transparent",
        )
        tool = payload["tools"][0]
        assert tool["model"] == "gpt-image-2.5-sunburst"
        assert tool["background"] == "transparent"

    def test_default_background_opaque(self):
        payload = codex_plugin._build_responses_payload(
...
class TestTransparencyDetection:
    @pytest.mark.parametrize("prompt,expected", [
        ...
        ("product cutout on white", True),
        ("a glass of transparent water", False),
        ("a busy street with signs", False),
        ("layers of atmosphere in a landscape", False),
    ...
    def test_detection(self, prompt, expected):
        assert bool(codex_plugin._TRANSPARENCY_PROMPT_RE.search(prompt)) is expected

    def test_generate_passes_transparent_background(self, provider, monkeypatch):
        monkeypatch.setattr(codex_plugin, "_read_codex_access_token", lambda: "codex-token")
        seen = {}
        original = codex_plugin._collect_image_b64

        def _collect(token, **kw):
            seen.update(kw)
            return {"b64": _b64_png(), "source": "final"}

        monkeypatch.setattr(codex_plugin, "_collect_image_b64", _collect)
        result = provider.generate("a corgi sticker with transparent background")
        assert result["success"] is True
