...
class TestZaiProviderIdentity:
    ...
    def test_auto_detect_opt_out(self):
        """The implicit plugin-provider walk must skip zai — ZAI_API_KEY is
        shared with the model provider, so key presence alone must not
        route never-configured installs into subscription-credit draws."""
        from plugins.web.zai.provider import ZaiWebSearchProvider
        assert ZaiWebSearchProvider().auto_detect is False
