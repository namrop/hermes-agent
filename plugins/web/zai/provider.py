...
class ZaiWebSearchProvider(WebSearchProvider):
    ...
    # Explicit-selection only. ZAI_API_KEY is shared with the Z.AI model
    # provider, so key presence alone must not route never-configured
    # installs into subscription-credit draws through the implicit
    # plugin-provider walk. Reachable via web.search_backend /
    # web.extract_backend / web.backend set to "zai".
