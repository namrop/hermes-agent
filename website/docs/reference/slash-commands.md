...
| `/model [provider:model]` | Show or change the model. Supports provider switches (`/model zai:glm-5`), custom endpoints (`/model custom:model`), named custom providers (`/model custom:local:qwen`), auto-detect (`/model custom`), and user-defined aliases (`/model fav`, `/model grok` — see [Custom model aliases](#custom-model-aliases)). In gateway chats, bare `/model default` clears only this channel/thread's explicit model override and pending one-turn selection; it preserves session ID, transcript, persona, and channel/global configuration, then restores normal channel-default → global-default/fallback policy. It rejects `--global` and `--once`; `/model default --provider <name>` remains a deliberate model selection. Use `--global` to persist a normal switch to config.yaml. **Note:** `/model` can only switch between already-configured providers. To add a new provider or set up API keys, use `hermes model` from your terminal (outside the chat session). **Cost note:** a mid-session model switch resets the prompt cache (the cache key includes the model), so the next message re-reads the whole conversation at full input price. |
